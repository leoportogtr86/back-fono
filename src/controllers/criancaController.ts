  ...
  try {
    ...
    if (!crianca) {
      return res
        ...
        .json({
          error:
            'Criança não encontrada ou não pertence ao fonoaudiólogo logado',
        });
    }
    res.json(crianca);
  } catch (error) {
...
export const updateCriancaHandler = async (req: Request, res: Response) => {
  ...
  const {
    ...
    responsavel_nome,
    responsavel_contato,
  }: IUpdateCriancaData = req.body;
  // @ts-ignore
  const fonoaudiologoId = req.user.id; // Obtém o ID do fonoaudiólogo logado
  try {
    const updatedCrianca = await updateCrianca(
      Number(id),
      {
        nome,
        data_nascimento,
